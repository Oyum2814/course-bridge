...
const Dashboard:React.FC<DashboardProps> = ({user}) => {
    ...
    return ( 
        <div className={clsx("flex flex-col w-full h-screen px-4 py-8 overflow-y-auto bg-[#D9D9D9] dark:bg-gray-900 border-r rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700")}>
            <a href="#" className="mx-auto text-2xl dark:text-white font-[600] ">
                Course Bridge
            </a>

            <div className="flex flex-col items-center mt-6 -mx-2">
                <Image width={30} height={45} className="object-cover w-24 h-24 mx-2 rounded-full"
                 src={user?.image||'https://images.unsplash.com/photo-1531427186611-ecfd6d936c79?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80'} alt="avatar" />
                <h4 className="mx-2 mt-2 font-medium text-gray-800 dark:text-gray-200">{user?.name}</h4>
                <p className="mx-2 mt-1 text-sm font-medium text-gray-600 dark:text-gray-400">{user?.role}</p>
            </div>
